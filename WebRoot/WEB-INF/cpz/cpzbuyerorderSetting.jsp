...
$(document).on('ready', function() {
	...
	var userphone='${entity.userphone}';
	$("#userphone").val(userphone);
	var orderstatus='${entity.orderstatus}';
	$("#orderstatus").val(orderstatus);
	var totalmoney='${entity.totalmoney}';
	$("#totalmoney").val(totalmoney);
	var paymoney='${entity.paymoney}';
	$("#paymoney").val(paymoney);
	var paystatus='${entity.paystatus}';
	$("#paystatus").val(paystatus);
	var shopid='${entity.shopid}';
	$("#shopid").val(shopid);
	var marketid='${entity.marketid}';
	$("#marketid").val(marketid);
	var shiptype='${entity.shiptype}';
	...
	var detailaddr='${entity.detailaddr}';
	$("#detailaddr").val(detailaddr);
	var invoicetype='${entity.invoicetype}';
	$("#invoicetype").val(invoicetype);
	var invoicetitle='${entity.invoicetitle}';
	$("#invoicetitle").val(invoicetitle);
	var orderremark='${entity.orderremark}';
	$("#orderremark").val(orderremark);
	var getproducttime='${entity.getproducttime}';
	$("#getproducttime").val(getproducttime);
	var booktime='${entity.booktime}';
	$("#booktime").val(booktime);
	var paytime='${entity.paytime}';
	$("#paytime").val(paytime);
	var channelno='${entity.channelno}';
	$("#channelno").val(channelno);
	var remark1='${entity.remark1}';
	$("#remark1").val(remark1);
	var remark2='${entity.remark2}';
	$("#remark2").val(remark2);
...
<body>
 <form action="CpzBuyerOrderAction!doUpdate" method="post" enctype="multipart/form-data" name="myForm">
...
<select id="orderstatus" name="entity.orderstatus" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${orderstatusSelectList}">	
						<c:choose>
							<c:when test="${item==entity.orderstatus}">								
...
<select id="paystatus" name="entity.paystatus" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${paystatusSelectList}">	
						<c:choose>
							<c:when test="${item==entity.paystatus}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
...
<select id="shiptype" name="entity.shiptype" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${shiptypeSelectList}">	
						<c:choose>
							<c:when test="${item==entity.shiptype}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
...
<select id="invoicetype" name="entity.invoicetype" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${invoicetypeSelectList}">	
						<c:choose>
							<c:when test="${item==entity.invoicetype}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
...
<select id="channelno" name="entity.channelno" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${channelnoSelectList}">	
						<c:choose>
							<c:when test="${item==entity.channelno}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
