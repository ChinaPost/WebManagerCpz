...
$(document).on('ready', function() {
	...
	$("#orderno").val(orderno);
	var refundreason='${entity.refundreason}';
	$("#refundreason").val(refundreason);
	var isconsultseller='${entity.isconsultseller}';
	$("#isconsultseller").val(isconsultseller);
	var refundstyle='${entity.refundstyle}';
	$("#refundstyle").val(refundstyle);
	var ordermoney='${entity.ordermoney}';
	$("#ordermoney").val(ordermoney);
	var applymoney='${entity.applymoney}';
	$("#applymoney").val(applymoney);
	var applytime='${entity.applytime}';
	$("#applytime").val(applytime);
	var refundtime='${entity.refundtime}';
...
<body>
 <form action="CpzBuyerRefundAction!doUpdate" method="post" enctype="multipart/form-data" name="myForm">
...
<select id="isconsultseller" name="entity.isconsultseller" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${isconsultsellerSelectList}">	
						<c:choose>
							<c:when test="${item==entity.isconsultseller}">								
...
<select id="refundstyle" name="entity.refundstyle" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${refundstyleSelectList}">	
						<c:choose>
							<c:when test="${item==entity.refundstyle}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
