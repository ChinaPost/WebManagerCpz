...
<title>测试</title>
	<script>
		function toAdd(){
			window.location.href="BbQqAction!toAdd?dd=${dd}";
		}
		...
		function dodel(mdd){
			...
			if(r){
				$.ajax({
					type:'POST',
					url:'BbQqAction!doDelete',
					data:{
dd:mdd					
},
					success:function(k){
							alert("删除成功！")
							window.location.href = "BbQqAction!index?dd=${dd}";
					},
					error : function() {
...
dd:searchInput					},
	 ...
	 divtext += '<td>' + data[i].b + '</td>';
								divtext += '<td ><a href="BbQqAction!toUpdate?dd='+data[i].dd+'"> [修改] </a>'
