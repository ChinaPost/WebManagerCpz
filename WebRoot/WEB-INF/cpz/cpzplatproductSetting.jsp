...
$(document).on('ready', function() {
	...
	var productcategoryid='${entity.productcategoryid}';
	...
	var tradingunit='${entity.tradingunit}';
	$("#tradingunit").val(tradingunit);
	var productcolor='${entity.productcolor}';
	$("#productcolor").val(productcolor);
	var isfresh='${entity.isfresh}';
	$("#isfresh").val(isfresh);
	var sourcearea='${entity.sourcearea}';
	$("#sourcearea").val(sourcearea);
	var iscanrefund='${entity.iscanrefund}';
	$("#iscanrefund").val(iscanrefund);
	var isneedspot='${entity.isneedspot}';
	$("#isneedspot").val(isneedspot);
	var productdetail='${entity.productdetail}';
	$("#productdetail").val(productdetail);
	var supportday='${entity.supportday}';
	$("#supportday").val(supportday);
	var channelno='${entity.channelno}';
	$("#channelno").val(channelno);
	var remakr1='${entity.remakr1}';
	$("#remakr1").val(remakr1);
	var remakr2='${entity.remakr2}';
	$("#remakr2").val(remakr2);
...
<body>
 <form action="CpzPlatProductAction!doUpdate" method="post" enctype="multipart/form-data" name="myForm">
...
<select id="producttype" name="entity.producttype" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${producttypeSelectList}">	
						<c:choose>
							<c:when test="${item==entity.producttype}">								
...
<select id="productstatus" name="entity.productstatus" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${productstatusSelectList}">	
						<c:choose>
							<c:when test="${item==entity.productstatus}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
...
<select id="tradingunit" name="entity.tradingunit" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${tradingunitSelectList}">	
						<c:choose>
							<c:when test="${item==entity.tradingunit}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
...
<select id="isfresh" name="entity.isfresh" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${isfreshSelectList}">	
						<c:choose>
							<c:when test="${item==entity.isfresh}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
...
<select id="iscanrefund" name="entity.iscanrefund" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${iscanrefundSelectList}">	
						<c:choose>
							<c:when test="${item==entity.iscanrefund}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
...
<select id="isneedspot" name="entity.isneedspot" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${isneedspotSelectList}">	
						<c:choose>
							<c:when test="${item==entity.isneedspot}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
