...
<title>结算表</title>
	<script>
		function toAdd(){
			window.location.href="GspSettlementAction!toAdd?settid=${settid}";
		}
		...
		$(document).on('ready', function() {
			var settid="${settid}";
			$("#settid").val(settid);
			getAll('GspSettlementAction!list');
		});
		...
		function dodel(msettid){
			var r=confirm("确定要删除吗？");
			if(r){
				$.ajax({
					type:'POST',
					url:'GspSettlementAction!doDelete',
					data:{
settid:msettid					
},
					success:function(k){
							alert("删除成功！")
							window.location.href = "GspSettlementAction!index?settid=${settid}";
					},
					error : function() {
...
settid:searchInput					},
	 ...
	 divtext += '<td>' + data[i].settsn + '</td>';
	 divtext += '<td>' + data[i].status + '</td>';
	 divtext += '<td>' + data[i].sumprice + '</td>';
	 divtext += '<td>' + data[i].sellerid + '</td>';
								divtext += '<td ><a href="GspSettlementAction!toUpdate?settid='+data[i].settid+'"> [修改] </a>'
								divtext +='|<a href="javascript:void(0);" onclick="dodel('+data[i].settid+')"> [删除] </a></td>';
								divtext += '</tr>';
							}
...
<body style="overflow:auto">
	...
	<input type="hidden" id="settid" name="settid" value="" />
	结算表：<input type="text" id="searchInput" style="margin-left:10px;width:100px;height:20px; "/>
	      ...
	      <tr>
	        ...
	        <table id="newtable" width="100%">
	          <thead class=trhead id="tblHeader">
				...
				<tr> 				<th  style="text-align:center;">订单号</th>
				<th  style="text-align:center;">结算流水</th>
