...
$(document).on('ready', function() {
	...
	var userid='${userid}';
	$("#userid").val(userid);
	var moileno='${entity.moileno}';
	$("#moileno").val(moileno);
	var nickname='${entity.nickname}';
	$("#nickname").val(nickname);
	var sex='${entity.sex}';
	$("#sex").val(sex);
	var headurl='${entity.headurl}';
	$("#headurl").val(headurl);
	var cstmlevel='${entity.cstmlevel}';
	$("#cstmlevel").val(cstmlevel);
...
<body>
 <form action="CpzBuyerMsgAction!doUpdate" method="post" enctype="multipart/form-data" name="myForm">
...
<select id="sex" name="entity.sex" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					...
					<tr >
						...
						<td>
						<img src=".${entity.headurl }" style="display: block;width:40px;height:30px"/>
						    <input type="hidden" id="headurl" name="entity.headurl"  value="${entity.headurl }"/>
...
<select id="cstmlevel" name="entity.cstmlevel" class="form-control" style="width: 187px;height:28px;margin-bottom:10px;">
				<option value="">请选择</option>
					<c:forEach var="item" items="${cstmlevelSelectList}">	
						<c:choose>
							<c:when test="${item==entity.cstmlevel}">								
								<option value='${fn:substringBefore(item,"-")}' selected="selected">${fn:substringAfter(item,"-")} </option>
							</c:when>
							<c:otherwise>
							<option value='${fn:substringBefore(item,"-")}'>${fn:substringAfter(item,"-")}</option>
